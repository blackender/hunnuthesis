...
为方便使用及更好地展示\LaTeX{}排版的优秀特性，hunnuthesis的框架和文件体系进行了细致地处理，尽可能地对各个功能和板块进行了模块化和封装，对于初学者来说，众多的文件目录也许一开始让人觉得有些无所适从，但阅读完下面的使用说明后，会发现原来使用思路是简单而清晰的，而且，当对\LaTeX{}有一定的认识和了解后，会发现其相对Word类排版系统极具吸引力的优秀特性。所以，如果是初学者，请不要退缩，请稍加尝试和坚持，以领略到\LaTeX{}的非凡魅力，并可以通过阅读相关资料如\LaTeX{} Wikibook \citep{wikibook2014latex} 来完善自己的使用知识。

\section{先试试效果}

\begin{enumerate}
    \item 安装软件：根据所用操作系统和章节~\ref{sec:system}中的信息安装\LaTeX{}编译环境。
    \item 获取模板：下载 \href{https://github.com/blackender/hunnuthesis}{hunnuthesis} 模板并解压。hunnuthesis模板不仅提供了相应的类文件，同时也提供了包括参考文献等在内的完成学位论文的一切要素，所以，下载时，推荐下载整个hunnuthesis文件夹，而不是单独的文档类。
    \item 编译模板：
        \begin{enumerate}
...
编译完成即可获得本PDF说明文档。而这也完成了学习使用hunnuthesis撰写论文的一半进程。什么？这就学成一半了，这么简单？？？，是的，就这么简单！
...
包含hunnuthesis文档类的定义文件和配置文件，通过对它们的修改可以实现特定的模版设定。

\begin{enumerate}
    \item hunnuthesis.cls：文档类定义文件，论文的最核心的格式即通过它来定义的。
    \item hunnuthesis.cfg：文档类配置文件，设定如目录显示为“目~录”而非“目录”。
...
文件夹内为论文的所有实体内容，正常情况下，这也是\textbf{使用hunnuthesis撰写学位论文时，主要关注和修改的一个位置，注：所有文件都必须采用UTF-8编码，否则编译后将出现乱码文本}，详细分类介绍如下：
...
用于放置论文中所需要的图类文件，支持格式有：.jpg, .png, .pdf。其中，\verb|ucas_logo.pdf|为湖南师大校徽。不建议为各章节图片建子目录，即使图片众多，若命名规则合理，图片查询亦是十分方便。
...
制图制表的更多范例，请见 \href{https://github.com/mohuangrui/hunnuthesis/wiki}{hunnuthesis 知识小站} 和 \href{https://en.wikibooks.org/wiki/LaTeX/Tables}{WiKibook Tables}。
...
1）使用Google Scholar搜索Document Preparation System，在目标条目下点击Cite，展开后选择Import into BibTeX打开此文章的BibTeX索引信息，将它们copy添加到ref.bib文件中（此文件位于Reference文件夹下）。
...
不同文献样式和引用样式，如著者-出版年制（authoryear）、顺序编码制（numbers）、上标顺序编码制（super）可在Thesis.tex中对artratex.sty调用实现，详见 \href{https://github.com/mohuangrui/hunnuthesis/wiki}{hunnuthesis 知识小站之文献样式}
...
\begin{enumerate}
    \item 模板每次发布前，都已在Windows，Linux，MacOS系统上测试通过。下载模板后，若编译出现错误，则请见 \href{https://github.com/mohuangrui/hunnuthesis/wiki}{hunnuthesis知识小站} 的 \href{https://github.com/mohuangrui/hunnuthesis/wiki/%E7%BC%96%E8%AF%91%E6%8C%87%E5%8D%97}{编译指南}。
    ...
    \item 模版的设计可能地考虑了适应性。致谢等所有条目都是通过最为通用的
        ...
        来显式实现的 (请观察Backmatter.tex)，从而可以随意添加，放置，和修改，如同一般章节。对于图表目录名称则可在hunnuthesis.cfg中进行修改。
    ...
    \item 文档内字体切换方法：
        \begin{itemize}
            \item 宋体：湖南师大论文模板hunnuthesis 或 \textrm{湖南师大论文模板hunnuthesis}
            \item 粗宋体：{\bfseries 湖南师大论文模板hunnuthesis} 或 \textbf{湖南师大论文模板hunnuthesis}
            \item 黑体：{\sffamily 湖南师大论文模板hunnuthesis} 或 \textsf{湖南师大论文模板hunnuthesis}
            \item 粗黑体：{\bfseries\sffamily 湖南师大论文模板hunnuthesis} 或 \textsf{\bfseries 湖南师大论文模板hunnuthesis}
            \item 仿宋：{\ttfamily 湖南师大论文模板hunnuthesis} 或 \texttt{湖南师大论文模板hunnuthesis}
            \item 粗仿宋：{\bfseries\ttfamily 湖南师大论文模板hunnuthesis} 或 \texttt{\bfseries 湖南师大论文模板hunnuthesis}
            \item 楷体：{\itshape 湖南师大论文模板hunnuthesis} 或 \textit{湖南师大论文模板hunnuthesis}
            \item 粗楷体：{\bfseries\itshape 湖南师大论文模板hunnuthesis} 或 \textit{\bfseries 湖南师大论文模板hunnuthesis}
        \end{itemize}
\end{enumerate}
